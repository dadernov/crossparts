...
from app.groups import BRAKE_DISCS, BRAKE_HOSES, BRAKE_PADS
...
HOSE = {"articleNumber": "24.5143-0564.3", "genericArticles": [
    {"genericArticleDescription": "Brake Hose"}],
    "oemNumbers": [{"mfrName": "SEAT", "articleNumber": "1K0 611 701 K"}]}
...
def test_group_detection_and_payload_follow_ate_public_contract():
    ...
    assert AteSource.article_group(HOSE) == BRAKE_HOSES
...
def test_candidate_is_scoped_to_confirmed_brake_groups():
    settings = Settings(_env_file=None, enabled_sources="", browser_fallback=False,
        pilot_rules=json.dumps({"ate": {"groups": [BRAKE_PADS, BRAKE_DISCS, BRAKE_HOSES],
    ...
    try:
        ...
        assert [source.key for source in registry.resolve(None, BRAKE_HOSES, tenant="catalogue-review")] == ["ate"]
    finally:
