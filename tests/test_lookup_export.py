...
async def test_single_lookup_export_returns_canonical_excel_numbers():
    ...
    assert first_variant[0] == (
        "Номер ОЕ (запрос)", "Бренд аналога", "Номер аналога", "Раздел", "Источники",
    )
    assert first_variant[1][2] == "P30122A"
    assert second_variant[0] == (
        "Товарная группа", "Номер ОЕ (запрос)", "Кол-во", "ОЕМ/Афтермаркет", "Все кроссы",
    )
    assert second_variant[1][4] == "P30122A"
