...
`login:password,another:password`; тестовая учётная запись — `0` / `0`. API
по-прежнему поддерживает `X-API-Key` (пары «ключ:тенант» в `CP_API_KEYS`).
Задания изолированы по тенанту.

На аккаунт доступно 1 000 поисковых позиций (`CP_REQUESTS_PER_ACCOUNT`). Один
поиск номера — одна позиция; Excel-файл расходует столько позиций, сколько в
нём OE-номеров. Каждый поиск обращается к каталогам заново, без использования
...
Скрытый круговой поиск по найденным кроссам выключен по умолчанию.
Включение через `CP_CIRCULAR_SEARCH_ENABLED=true`, лимиты и порядок работы:
[инструкция](docs/circular-search.md).
